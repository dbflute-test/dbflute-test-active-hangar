...
/**
 * The definition of empty classification.
 * @author FreeGen
 */
public interface EmptyCDef extends Classification {
    ...
    public enum DefMeta implements ClassificationMeta {
        ...
        public static OptionalThing<EmptyCDef.DefMeta> find(String classificationName) { // instead of valueOf()
            if (classificationName == null) { throw new IllegalArgumentException("The argument 'classificationName' should not be null."); }
            return OptionalThing.ofNullable(_nameMetaMap.get(classificationName.toLowerCase()), () -> {
                throw new ClassificationNotFoundException("Unknown classification: " + classificationName);
            });
        }
        public static EmptyCDef.DefMeta meta(String classificationName) { // old style so use find(name)
            return find(classificationName).orElse(null);
        }
    }

    public static class ZzzoneSlimmer<CLS extends EmptyCDef> {
