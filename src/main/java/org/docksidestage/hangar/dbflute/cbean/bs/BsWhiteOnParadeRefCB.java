...
/**
 * The base condition-bean of WHITE_ON_PARADE_REF.
 * @author DBFlute(AutoGenerator)
 */
public class BsWhiteOnParadeRefCB extends AbstractConditionBean {

    // ===================================================================================
    //                                                                           Attribute
    //                                                                           =========
    protected WhiteOnParadeRefCQ _conditionQuery;

    // ===================================================================================
    //                                                                         Constructor
    //                                                                         ===========
    public BsWhiteOnParadeRefCB() {
     ...
     * @return this. (NotNull)
     */
    public WhiteOnParadeRefCB acceptPK(Long refId) {
        assertObjectNotNull("refId", refId);
        BsWhiteOnParadeRefCB cb = this;
        cb.query().setRefId_Equal(refId);
        return (WhiteOnParadeRefCB)this;
    }
     ...
     */
    public WhiteOnParadeRefCQ query() {
        assertQueryPurpose(); // assert only when user-public query
        return doGetConditionQuery();
    }

    public WhiteOnParadeRefCQ xdfgetConditionQuery() { // public for parameter comment and internal
        return doGetConditionQuery();
    }

    protected WhiteOnParadeRefCQ doGetConditionQuery() {
    ...
    protected WhiteOnParadeRefCQ createLocalCQ() {
        return xcreateCQ(null, getSqlClause(), getSqlClause().getBasePointAliasName(), 0);
    }

    protected WhiteOnParadeRefCQ xcreateCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        WhiteOnParadeRefCQ cq = xnewCQ(childQuery, sqlClause, aliasName, nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected WhiteOnParadeRefCQ xnewCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        return new WhiteOnParadeRefCQ(childQuery, sqlClause, aliasName, nestLevel);
    }
     ...
     */
    public void union(UnionQuery<WhiteOnParadeRefCB> unionCBLambda) {
        final WhiteOnParadeRefCB cb = new WhiteOnParadeRefCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final WhiteOnParadeRefCQ cq = cb.query(); query().xsetUnionQuery(cq);
    }
     ...
     */
    public void unionAll(UnionQuery<WhiteOnParadeRefCB> unionCBLambda) {
        final WhiteOnParadeRefCB cb = new WhiteOnParadeRefCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final WhiteOnParadeRefCQ cq = cb.query(); query().xsetUnionAllQuery(cq);
    }
     ...
     *     ... = <span style="color: #553000">whiteOnParadeRef</span>.<span style="color: #CC4747">getWhiteOnParade()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
     */
    public void setupSelect_WhiteOnParade() {
        assertSetupSelectPurpose("whiteOnParade");
    ...
    public static class HpSpecification extends HpAbstractSpecification<WhiteOnParadeRefCQ> {
         ...
         * @return The information object of specified column. (NotNull)
         */
        public SpecifiedColumn columnNullableFkOnParadeId() { return doColumn("NULLABLE_FK_ON_PARADE_ID"); }
        /**
         * NULLABLE_FK_TO_MANY_ID: {IX, DECIMAL(16), FK to WHITE_ON_PARADE_NULLABLE_TO_MANY}
         * @return The information object of specified column. (NotNull)
         */
        public SpecifiedColumn columnNullableFkToManyId() { return doColumn("NULLABLE_FK_TO_MANY_ID"); }
        ...
        public WhiteOnParadeCB.HpSpecification specifyWhiteOnParade() {
            assertRelation("whiteOnParade");
            if (_whiteOnParade == null) {
                _whiteOnParade = new WhiteOnParadeCB.HpSpecification(_baseCB
                    , xcreateSpQyCall(() -> _qyCall.has() && _qyCall.qy().hasConditionQueryWhiteOnParade()
                                    , () -> _qyCall.qy().queryWhiteOnParade())
                    , _purpose, _dbmetaProvider, xgetSDRFnFc());
                if (xhasSyncQyCall()) { // inherits it
                    _whiteOnParade.xsetSyncQyCall(xcreateSpQyCall(
                        () -> xsyncQyCall().has() && xsyncQyCall().qy().hasConditionQueryWhiteOnParade()
                      , () -> xsyncQyCall().qy().queryWhiteOnParade()));
                }
            }
         ...
         */
        public HpSDRFunction<WhiteOnParadeRefCB, WhiteOnParadeRefCQ> myselfDerived() {
            assertDerived("myselfDerived"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<WhiteOnParadeRefCB> sq, WhiteOnParadeRefCQ cq, String al, DerivedReferrerOption op)
     ...
     */
    public WhiteOnParadeRefCB dreamCruiseCB() {
        WhiteOnParadeRefCB cb = new WhiteOnParadeRefCB();
        cb.xsetupForDreamCruise((WhiteOnParadeRefCB) this);
        return cb;
    }
     ...
     */
    public HpColQyOperand<WhiteOnParadeRefCB> columnQuery(final SpecifyQuery<WhiteOnParadeRefCB> colCBLambda) {
        return xcreateColQyOperand((rightSp, operand) -> {
            return xcolqy(xcreateColumnQueryCB(), xcreateColumnQueryCB(), colCBLambda, rightSp, operand);
        });
    }

    protected WhiteOnParadeRefCB xcreateColumnQueryCB() {
        WhiteOnParadeRefCB cb = new WhiteOnParadeRefCB();
        cb.xsetupForColumnQuery((WhiteOnParadeRefCB)this);
        return cb;
    }
     ...
     */
    public void orScopeQuery(OrQuery<WhiteOnParadeRefCB> orCBLambda) {
        xorSQ((WhiteOnParadeRefCB)this, orCBLambda);
    }
     ...
     */
    public void orScopeQueryAndPart(AndQuery<WhiteOnParadeRefCB> andCBLambda) {
        xorSQAP((WhiteOnParadeRefCB)this, andCBLambda);
    }
    ...
    protected void xprepareSyncQyCall(ConditionBean mainCB) {
        final WhiteOnParadeRefCB cb;
        if (mainCB != null) {
            cb = (WhiteOnParadeRefCB)mainCB;
        } else {
            cb = new WhiteOnParadeRefCB();
        }
    ...
    protected String xgetConditionBeanClassNameInternally() { return WhiteOnParadeRefCB.class.getName(); }
    protected String xgetConditionQueryClassNameInternally() { return WhiteOnParadeRefCQ.class.getName(); }
